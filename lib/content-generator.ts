...
export function generateContent(topic: Topic): GeneratedResult {
  ...
  const noteBody = details?.noteBody
    ? // 専門コンテンツがある場合: リード + 本文 + データ + 実践 + まとめ + 著者
      `${topic.hook}\n\n` +
    ...
    : // フォールバック: コンテンツ未生成の案内
      `━━━━━━━━━━━━━━━\n` +
      `このトピックのnote記事はまだ生成されていません。\n\n` +
      `高品質なコンテンツを生成するには、Claude Codeで以下を実行してください：\n\n` +
      `/sns ${topic.title}\n\n` +
      `━━━━━━━━━━━━━━━\n\n` +
      `テーマ: ${topic.title}\n` +
      `フック: ${topic.hook}\n` +
...
function generateReelHtml(topic: Topic, reelData: string[]): string {
  ...
  const details = topicDetails[topic.id];
  // reelHookがなければタイトルを「：」「、」「—」で分割し先頭部分を使う（1行6文字制限対策）
  const rawTitle = topic.title.split(/[：:、—]/)[0];
  const shortHook = details?.reelHook || (rawTitle.length > 6 ? rawTitle.slice(0, 6) + "\n" + rawTitle.slice(6, 12) : rawTitle);
...
<div class="scene" id="s1">
  <div class="hook-text">${escHtml(shortHook).split("\n").map(l => `<span style="display:block;white-space:nowrap">${l}</span>`).join("")}</div>
